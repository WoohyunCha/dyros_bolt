...
struct SharedMessage {
    double q[MODEL_DOF_shm], virtual_q[6], qdot[MODEL_DOF_shm], virtual_qdot[6], torque_[MODEL_DOF_shm], command[MODEL_DOF_shm], base_pos[3], base_quat[4];
    float sim_time;
    int ctrl_mode;
...
public:
    ...
    void receiveData(const double* q_, const double* virtual_q_, const double* qdot_, const double* virtual_qdot_, const double* torque_, const double* base_pos_, const double* base_quat_, const double& ctrl_time_); // From sim, to shm
    void writeData(VectorQd& q_, Vector6d& virtual_q_, VectorQd& qdot_, Vector6d& virtual_qdot_, VectorQd& torque, Eigen::Vector3d& base_pose_, Eigen::Quaterniond& base_quat_, double& sim_time_); // From shm, to controller
